...
public class DatasetP2Generator extends DatasetBuilder<Dataset<Timeoff, Lesson>, DatasetConverter<Int2IntLinkedOpenHashMap>>
{
    ...
    protected void generateLessonSet()
    {
        try
        {
            ...
            int lesson_extra_counter = lesson_size - 1;
            ...
            dataset.lessons = new Lesson[size];
            ...
            for(int result_set_counter = -1; db_component.result_set.next(); )
            {
            ...
            for(int lesson_counter = lesson_size + lesson_extra_size - 1, is = dataset.lessons.length; ++lesson_counter < is; )
            {
                dataset.lessons[lesson_counter] = new Lesson(0, 0, 1, 0, 0, 0);
            }
        }
    ...
    protected void generateLessonAvailableClassroom()
    {
        try
        {
            DBComponent                                db_component = super.db_component;
            ...
            String query = "SELECT `lesson`.`id`, GROUP_CONCAT(DISTINCT `lesson_available_classroom`.`classroom` ORDER BY `lesson_available_classroom`.`classroom` ASC SEPARATOR ' ') AS 'available_classroom' FROM `lesson` LEFT OUTER JOIN `subject` ON `lesson`.`subject` = `subject`.`id` LEFT OUTER JOIN `lesson_available_classroom` ON `lesson_available_classroom`.`lesson` = `lesson`.`id` WHERE `subject`.`school` = ? GROUP BY `lesson`.`id` ORDER BY `lesson`.`id`, `lesson_available_classroom`.`classroom` ASC";
            db_component.statement = db_component.connection.prepareStatement(query);
            db_component.statement.setInt(1, dataset.school);
            db_component.result_set = db_component.statement.executeQuery();
            ...
            {
                int      lesson_index        = encoder.lessons.get(db_component.result_set.getInt("id"));
                String[] available_classroom = db_component.result_set.getString("available_classroom").split(" ");
                for(int available_classroom_counter = -1, avl_s = available_classroom.length; ++available_classroom_counter < avl_s; )
                {
                    dataset.lessons[lesson_index].available_classroom[available_classroom_counter] = Integer.parseInt(available_classroom[available_classroom_counter]);
                }
            }
        }
        catch(SQLException ignore)
    ...
    @Override protected void generateClassrooms()
    {
        try
        {
            ...
            dataset.classrooms = new Timeoff[size];
            encoder.classrooms = new Int2IntLinkedOpenHashMap(size);
            decoder.classrooms = new Int2IntLinkedOpenHashMap(size);

            for(int result_set_counter = 0; db_component.result_set.next(); ++result_set_counter)
            {
                dataset.classrooms[result_set_counter] = new Timeoff(dataset.active_days.length, dataset.active_periods.length);
            ...
            while(db_component.result_set.next())
            {
                dataset.classrooms[encoder.classrooms.get(db_component.result_set.getInt("classrooms"))].add(encoder.active_days.get(db_component.result_set.getInt("day")), encoder.active_periods.get(db_component.result_set.getInt("period")), db_component.result_set.getDouble("value"));
            }
        }
        catch(SQLException ignored)
        {
    ...
    @Override protected void generateClasses()
    {
        try
        {
            ...
            for(int i = 0; db_component.result_set.next(); ++i)
            {
                dataset.classes[i] = new Timeoff(dataset.active_days.length, dataset.active_periods.length);
                encoder.classes.put(db_component.result_set.getInt("id"), i);
                decoder.classes.put(i, db_component.result_set.getInt("id"));
            }
    ...
    @Override protected void generateActiveDays()
    {
        try
        {
            ...
            decoder.active_days = new Int2IntLinkedOpenHashMap(size);
            ...
            for(int result_set_counter = 0; db_component.result_set.next(); ++result_set_counter)
            {
                ...
                decoder.active_days.put(result_set_counter, db_component.result_set.getInt("id"));
            }
        }
        catch(SQLException ignored)
        {
